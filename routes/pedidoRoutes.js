...
router.post('/pedido/nuevo', ensureAuthenticated, checkRole(['superadministrador', 'administrador']), pedidoController.crearPedido);
router.post('/pedido/:id/cerrar', ensureAuthenticated, checkRole(['superadministrador', 'administrador', 'cajero']), pedidoController.cerrarCuenta);

// NUEVA RUTA: Cancelación selectiva por ítems y auditoría de inventario
router.post('/pedido/:id/cancelar-parcial-o-total', ensureAuthenticated, checkRole(['superadministrador', 'administrador', 'cajero']), pedidoController.cancelarServicio);

// Ahora recibe un lote completo de productos de la orden
router.post('/pedido/:id/enviar-orden', pedidoController.enviarOrden);
